...
(function (RSADemo) {
    ...
    const BigInteger = Lib.BigInteger;
    {
        function initialize(checkParam) {
            ...
            Dom.getInput("demo_id_4").value = checkParam("m", "" + m);
            return { p, q, m };
        }
        function reload() {
            const p = BigInteger.parse(Dom.getInput("demo_id_2").value);
            const q = BigInteger.parse(Dom.getInput("demo_id_3").value);
            const m = BigInteger.parse(Dom.getInput("demo_id_4").value);
            return { p, q, m };
        }
        function update(show_branch, set_value, vars) {
            const { p, q, m } = vars;
            const LIMIT = 200;
            const variables = ["p", "q", "m"];
            function calc_coprimes(n) {
                let list = [BigInteger.ONE];
                let limited = false;
            ...
            for (let i = 0; i < items2.length; i++) {
                ...
                tbody3.appendChild(Dom.tr(Dom.td({ className: "top" }, "$pq 未満の" + sym + "の倍数:$"), Dom.td(RSADemo.format_set(exp1, limited)), Dom.td("$(" + sym + " に " + sym2 + " 未満の自然数を掛けたもの" + v2.sub(1) + "個)$")));
                tbody3.appendChild(Dom.tr(Dom.td({ className: "top right" }, "$=$"), Dom.td(RSADemo.format_set(exp2, limited))));
            }
            set_value("demo_id_23", bq2);
            const num_coprimes = phi(p.mul(q));
            const length_match = p.sub(1).mul(q.sub(1)).equals(num_coprimes);
            const coprime_set = RSADemo.format_set(coprimes, limited);
            ...
            set_value("demo_id_43", RSADemo.format_set(x_list0, limited));
            set_value("demo_id_44", RSADemo.format_set(x_list, limited));
            const exp_tbody = Dom.elem("tbody", Dom.elem("tr", Dom.td("$C(" + n + ")$"), Dom.td("$=$"), Dom.td(RSADemo.format_set(coprimes, limited))), Dom.elem("tr", Dom.td("$X(" + n + ", " + m + ")$"), Dom.td("$=$"), Dom.td(RSADemo.format_set(x_list_sorted, limited))));
            ...
            if (x_list.length == x_list_uniq.length) {
                ...
                exp_tbody.appendChild(Dom.tr(Dom.td(), Dom.td("$=$"), Dom.td(RSADemo.format_set(x_list_uniq, limited))));
                x_list_match = false;
            }
