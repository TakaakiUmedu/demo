...
(function (RSADemo) {
    ...
    const BigInteger = Lib.BigInteger;
    {
        ...
        function update(show_branch, set_value, vars) {
            const { p, q, e } = vars;
            function str_to_bi_list(str, law) {
            ...
            function bi_list_to_str(list, law) {
                let num = BigInteger.ZERO;
                for (let i = list.length - 1; i >= 0; i--) {
            ...
            function create_n_digits_random_number(max_int, n) {
                while (true) {
            ...
            function create_random_primes(event) {
            ...
            const d = RSADemo.calcInv(vars.e, L);
            ...
            }
            {
                ...
                function update(show_branch, set_value, vars) {
                    ...
                    for (let i = 0; i < m.length; i++) {
                        const j = i + 1;
                        c[i] = m[i].pow(e, N);
                        m_tbody.appendChild(RSADemo.tr("m_" + j, "=", "" + m[i]));
                        c_tbody.appendChild(RSADemo.tr("c_" + j, "=", "m_" + j + "^e", "\\%", "N", "=", makeExpA(m[i], e, N), "=", c[i].toString()));
                        if (d !== null) {
                            r[i] = c[i].pow(d, N);
                            if (!r[i].equals(m[i])) {
                                match = false;
                            }
                            r_tbody.appendChild(RSADemo.tr("r_" + j, "=", "c_" + j + "^d", "\\%", "N", "=", makeExpA(c[i], d, N), "=", r[i].toString()));
                        }
                    }
                    let d_strs = [];
                    if (d !== null) {
